...
//console.log('Form',diets)
  ...
  const handleCheckboxChange = (event, dietId) => {
    ...
    if (isChecked) {
      ...
      setErrors(validate({ ...inputs,  diets: inputs.diets.filter((id) => id !== dietId), }));
    }
  };
  ...
  return (
    <div>
      ...
      <form onSubmit={handleSubmit}>
        ...
        />
        {errors.healthScore && <div className="danger">{errors.healthScore}</div>}
        <br />
        ...
        <br />*{inputs.diets.length}+
        {inputs.diets?.map((diet, i) => 
          <div key={i}>{diet}</div>
          )}
